...
5. **Inject only into `text/html` responses with 2xx status.** Everything else passes through untouched. Operator `excludePaths` (TF: `exclude_paths`; CloudFront-style `*` and `?` wildcards, leading `/` optional, matched against the structurally normalized but NOT percent-decoded path) and responses carrying `X-Robots-Tag: noindex` or `none` pass through byte-identical with their normal caching, paying no lookup, no auto-registration, and no cache rewriting (a `<meta name="robots">` in the body is NOT visible to the Lambda@Edge origin-response layer; use `excludePaths` for those sections).
