...
5. **Inject only into `text/html` responses with 2xx status.** Everything else passes through untouched. Operator `excludePaths` (TF: `exclude_paths`, CloudFront-style `*` wildcards) and responses carrying `X-Robots-Tag: noindex` pass through byte-identical with their normal caching, paying no lookup, no auto-registration, and no cache rewriting (a `<meta name="robots">` in the body is NOT visible to the Lambda@Edge origin-response layer; use `excludePaths` for those sections).
...
- `404` = record missing; `429` = org rate limit (`Retry-After` header). Both → fail-open, serve original HTML. For public, non-credentialed requests with an explicit origin cache lifetime, the Lambda@Edge adapter shortens that lifetime to the next core/config retry and strips origin validators. Without an explicit `max-age`, `s-maxage`, `Expires`, or `no-cache`, it leaves the response untouched by default because it cannot see (and must not increase) the distribution's DefaultTTL — unless the operator sets the baked-config flag `capUninjectedTtl: true` (TF: `cap_uninjected_ttl`), which asserts a nonzero DefaultTTL so the same bounded retry Cache-Control may be applied to lifetime-less responses too (otherwise a single lookup timeout pins an uninjected page for the full DefaultTTL). Never enable it on a DefaultTTL=0 distribution. The core additionally records a short retry backoff after a `429`/error (`Retry-After` capped at 60 s; 10 s default) so page views don't hammer a rate-limited or down API.
