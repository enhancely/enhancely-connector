...
| **CloudFront**         | Lambda@Edge function (us-east-1), attached to **origin-response** | No environment variables at the edge → key via SSM Parameter Store or baked at deploy time; CloudFront cache in front.        |
